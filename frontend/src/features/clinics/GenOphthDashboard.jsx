...
const rxFields = [
  ...
  { label: 'Cylinder (CYL)', fields: ['odCyl', 'osCyl'] },
  { label: 'Axis', fields: ['odAxis', 'osAxis'] },
  { label: 'PD', fields: ['odPd', 'osPd'] },
];

export default function GenOphthDashboard() {
...
const [showFollowUpModal, setShowFollowUpModal] = useState(false);
  ...
  const queue = useClinicQueue('general-ophth', handleQueueSelect);
    ...
    >
      ...
      {patients.selectedPatient && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <ClinicSection title="Slit-Lamp Findings">
              <div className="space-y-3">
                <Input label="Lids / Lashes" placeholder="Normal / Blepharitis / Stye" value={slitLamp.lids} onChange={(e) => setSlitLamp((p) => ({ ...p, lids: e.target.value }))} />
                <Input label="Conjunctiva" placeholder="Clear / Injection / Follicles" value={slitLamp.conjunctiva} onChange={(e) => setSlitLamp((p) => ({ ...p, conjunctiva: e.target.value }))} />
                <Input label="Cornea" placeholder="Clear / Edema / Scar" value={slitLamp.cornea} onChange={(e) => setSlitLamp((p) => ({ ...p, cornea: e.target.value }))} />
                <Input label="Anterior Chamber" placeholder="Deep & quiet / Cells & flare" value={slitLamp.anteriorChamber} onChange={(e) => setSlitLamp((p) => ({ ...p, anteriorChamber: e.target.value }))} />
                <Input label="Iris" placeholder="Normal / Rubeosis / Synechiae" value={slitLamp.iris} onChange={(e) => setSlitLamp((p) => ({ ...p, iris: e.target.value }))} />
                <Input label="Lens" placeholder="Clear / NS / PSC / PCIOL" value={slitLamp.lens} onChange={(e) => setSlitLamp((p) => ({ ...p, lens: e.target.value }))} />
              </div>
            </ClinicSection>

            <ClinicSection title="Refraction Module">
              <div className="mb-4 grid grid-cols-3 gap-2 text-caption font-medium text-slate uppercase">
                <span></span>
                <span className="text-center">OD (Right)</span>
                <span className="text-center">OS (Left)</span>
              </div>
              {rxFields.map(({ label, fields }) => (
                <div key={label} className="grid grid-cols-3 gap-2 items-center mb-3">
                  <span className="text-body text-graphite">{label}</span>
                  <input
                    className="w-full px-3 py-2 bg-paper border border-silver rounded-lg text-body text-center touch-target"
                    value={refraction[fields[0]]}
                    onChange={(e) => setRefraction((p) => ({ ...p, [fields[0]]: e.target.value }))}
                    placeholder="—"
                  />
                  <input
                    className="w-full px-3 py-2 bg-paper border border-silver rounded-lg text-body text-center touch-target"
                    value={refraction[fields[1]]}
                    onChange={(e) => setRefraction((p) => ({ ...p, [fields[1]]: e.target.value }))}
                    placeholder="—"
                  />
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <ClinicSection title="Prescriptions">
              <PrescriptionWriter medications={medications} onChange={setMedications} clinicSlug="general-ophth" />
            </ClinicSection>
          </div>
              ...
              <Table
                columns={[
                  ...
                  { key: 'findings', label: 'VA', render: (r) => {
                    ...
                    if (!v?.visualAcuityOd && !v?.visualAcuityOs) return '-';
                    return [v.visualAcuityOd && `OD:${v.visualAcuityOd}`, v.visualAcuityOs && `OS:${v.visualAcuityOs}`].filter(Boolean).join(' ');
                  }},
                ]}
