...
import OrbitDiagram from '../../components/anatomical/OrbitDiagram';
...
export default function OrbitDashboard() {
  ...
  const records = useClinicalRecords('orbit');
  ...
  const queue = useClinicQueue('orbit', handleQueueSelect);
  ...
  const [orbitalMetrics, setOrbitalMetrics] = useState({
    proptosisOd: '', proptosisOs: '', verticalDystopia: '', horizontalDystopia: '', mrd1Od: '', mrd1Os: '',
  });

  const handleRegionSelect = (regionId) => {
    setSelectedRegion(regionId);
    setFindings((prev) => ({ ...prev, [regionId]: prev[regionId] ? null : { noted: true } }));
  };
  ...
  const resetForm = useCallback(() => {
    ...
    setFindings({});
    setOrbitalMetrics({ proptosisOd: '', proptosisOs: '', verticalDystopia: '', horizontalDystopia: '', mrd1Od: '', mrd1Os: '' });
    setSaveMessage('');
    ai.reset();
  }, [ai]);
  ...
  }, [patients.selectedPatient, vitals, symptoms, diagnosis, diagnosisIcd10, medications, soapNotes, findings, orbitalMetrics, records, resetForm]);

  return (
    <ClinicDashboardShell
      title="Orbit Clinic"
      subtitle="Orbital & Oculoplastics Assessment with AI-Assisted Diagnosis"
      actionButtons={
        showReferralBtn && (
    ...
    >
      ...
      {patients.selectedPatient && (
        <>
                ...
                {symptoms.map((symp, idx) => (
                  <div key={idx} className="bg-bone rounded-lg p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <Input label="Symptom" placeholder="e.g. Proptosis" value={symp.name} onChange={(e) => updateSymptom(idx, 'name', e.target.value)} />
                    ...
                    <Input label="Description" placeholder="Laterality (OD/OS/OU), quality, aggravating/relieving factors..." value={symp.description} onChange={(e) => updateSymptom(idx, 'description', e.target.value)} />
                  </div>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <ClinicSection title="Orbital Anatomy">
              <OrbitDiagram
              ...
              {selectedRegion && (
                <div className="mt-3 p-3 bg-lilac-bloom/10 rounded-lg">
                  <p className="text-sm font-medium text-graphite">Region {selectedRegion} Selected</p>
                  <p className="text-caption text-slate">Marked on orbital diagram. Document findings in the assessment below.</p>
                </div>
              )}
            </ClinicSection>

            <ClinicSection title="Orbital Metrics">
              <div className="grid grid-cols-2 gap-4">
                <Input label="Proptosis (OD)" placeholder="mm" value={orbitalMetrics.proptosisOd} onChange={(e) => setOrbitalMetrics((p) => ({ ...p, proptosisOd: e.target.value }))} />
                <Input label="Proptosis (OS)" placeholder="mm" value={orbitalMetrics.proptosisOs} onChange={(e) => setOrbitalMetrics((p) => ({ ...p, proptosisOs: e.target.value }))} />
                <Input label="Vertical Dystopia" placeholder="mm" value={orbitalMetrics.verticalDystopia} onChange={(e) => setOrbitalMetrics((p) => ({ ...p, verticalDystopia: e.target.value }))} />
                <Input label="Horizontal Dystopia" placeholder="mm" value={orbitalMetrics.horizontalDystopia} onChange={(e) => setOrbitalMetrics((p) => ({ ...p, horizontalDystopia: e.target.value }))} />
                <Input label="MRD1 (OD)" placeholder="mm" value={orbitalMetrics.mrd1Od} onChange={(e) => setOrbitalMetrics((p) => ({ ...p, mrd1Od: e.target.value }))} />
                <Input label="MRD1 (OS)" placeholder="mm" value={orbitalMetrics.mrd1Os} onChange={(e) => setOrbitalMetrics((p) => ({ ...p, mrd1Os: e.target.value }))} />
              </div>
            </ClinicSection>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <ClinicSection title="Prescriptions">
              <PrescriptionWriter medications={medications} onChange={setMedications} clinicSlug="orbit" />
            </ClinicSection>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <textarea value={soapNotes.subjective} onChange={(e) => setSoapNotes({ ...soapNotes, subjective: e.target.value })}
                    placeholder="Chief complaint, onset, laterality (OD/OS/OU), proptosis, diplopia, pain, trauma history, thyroid history..."
                ...
                <div>
                  ...
                  <textarea value={soapNotes.objective} onChange={(e) => setSoapNotes({ ...soapNotes, objective: e.target.value })}
                    placeholder="VA, proptosis (Hertel), ocular motility, eyelid position (MRD1), exophthalmometry, CT/MRI findings..."
                    className="w-full h-24 px-4 py-3 bg-paper border border-silver rounded-lg text-body text-obsidian placeholder:text-slate focus:outline-none focus:ring-2 focus:ring-lilac-bloom resize-none" />
                </div>
                <div>
                  ...
                  <textarea value={soapNotes.assessment} onChange={(e) => setSoapNotes({ ...soapNotes, assessment: e.target.value })}
                    placeholder="Diagnosis, orbital pathology type, activity/severity, vision-threatening factors..."
                    className="w-full h-24 px-4 py-3 bg-paper border border-silver rounded-lg text-body text-obsidian placeholder:text-slate focus:outline-none focus:ring-2 focus:ring-lilac-bloom resize-none" />
                </div>
                <div>
                  ...
                  <textarea value={soapNotes.plan} onChange={(e) => setSoapNotes({ ...soapNotes, plan: e.target.value })}
                    placeholder="Treatment plan (medical/surgical), follow-up interval, imaging needed, referrals (endocrinology/neurosurgery/ENT)..."
                    className="w-full h-24 px-4 py-3 bg-paper border border-silver rounded-lg text-body text-obsidian placeholder:text-slate focus:outline-none focus:ring-2 focus:ring-lilac-bloom resize-none" />
                </div>
              </div>
            </ClinicSection>
            ...
            <ClinicSection title="Patient History" className="mb-6">
              <Table
                columns={[
                  ...
                  { key: 'findings', label: 'Proptosis', render: (r) => {
                    const json = r.clinicSpecificJson;
                    if (!json?.orbitalMetrics) return '-';
                    const m = json.orbitalMetrics;
                    return [m.proptosisOd && `OD:${m.proptosisOd}`, m.proptosisOs && `OS:${m.proptosisOs}`].filter(Boolean).join(' ');
                  }},
                ]}
      ...
      {!patients.selectedPatient && records.stats && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
          ...
          <StatCard label="Orbit Cases" value={records.stats.totalPatients} variant="highlight" />
        </div>
      )}
