...
router.get('/tests', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_READ), async (req, res) => {
  try {
...
router.get('/tests/categories', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_READ), async (req, res) => {
  try {
...
router.post('/tests', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_CATALOG), async (req, res) => {
  try {
    ...
    if (err.code === 'P2002') return res.status(400).json({ message: 'Test code already exists' });
    res.status(500).json({ message: 'Internal server error' });
  }
});

router.put('/tests/:id', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_CATALOG), async (req, res) => {
  try {
...
router.delete('/tests/:id', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_CATALOG), async (req, res) => {
  try {
...
router.get('/panels', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_READ), async (req, res) => {
  try {
...
router.post('/panels', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_CATALOG), async (req, res) => {
  try {
    const { name, nameAr, testIds } = req.body;
    if (!name || !testIds?.length) return res.status(400).json({ message: 'name and testIds are required' });
...
router.delete('/panels/:id', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_CATALOG), async (req, res) => {
  try {
    await prisma.diagnosticPanel.update({ where: { id: req.params.id }, data: { isActive: false } });
    res.json({ success: true });
  } catch (err) {
    if (err.code === 'P2025') return res.status(404).json({ message: 'Panel not found' });
    res.status(500).json({ message: 'Internal server error' });
  }
});

router.get('/orders', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_READ), async (req, res) => {
  try {
...
router.get('/orders/:id', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_READ), async (req, res) => {
  try {
...
router.post('/orders', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_ORDER), async (req, res) => {
  try {
    const { patientId, fromClinicId, testIds, panelId, clinicalNotes, priority } = req.body;
    if (!patientId || !fromClinicId || (!testIds?.length && !panelId)) {
      return res.status(400).json({ message: 'patientId, fromClinicId, and testIds or panelId are required' });
    }
    ...
    if (panelId) {
      ...
      if (!panel) return res.status(400).json({ message: 'Panel not found' });
    ...
    const order = await prisma.diagnosticOrder.create({
      data: {
        ...
        clinicalNotes: clinicalNotes || null,         priority: typeof priority === 'number' ? priority : priority === 'URGENT' ? 1 : priority === 'STAT' ? 2 : 0,
...
router.patch('/orders/:id/claim', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_WRITE), async (req, res) => {
  try {
...
router.patch('/orders/:id/unclaim', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_WRITE), async (req, res) => {
  try {
    const order = await prisma.diagnosticOrder.update({
      where: { id: req.params.id },
...
router.patch('/orders/:id/status', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_WRITE), async (req, res) => {
  try {
...
router.put('/orders/:id/results', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_RESULTS), async (req, res) => {
  try {
    const { results } = req.body;
    if (!results?.length) return res.status(400).json({ message: 'results array is required' });
    const order = await prisma.diagnosticOrder.findUnique({
      where: { id: req.params.id },
      include: { tests: true },
    });
...
router.get('/results', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_READ), async (req, res) => {
  try {
...
router.get('/orders/:id/report', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_READ), async (req, res) => {
  try {
    const order = await prisma.diagnosticOrder.findUnique({
      where: { id: req.params.id },
      include: ORDER_INCLUDE,
...
router.post('/checkout', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_ORDER), async (req, res) => {
  try {
    ...
    console.error('Lab checkout error:', err);
    res.status(500).json({ message: 'Internal server error' });
  }
});

router.get('/stats', authenticate, requirePermission(PERMISSIONS.DIAGNOSTICS_READ), async (req, res) => {
  try {
