...
router.post('/', authenticate, requirePermission(PERMISSIONS.INVENTORY_WRITE), asyncHandler(async (req, res) => {
  ...
  const item = await prisma.inventoryItem.create({
    data: {
      name, nameAr, sku, category: 'hospital', quantity: initialQuantity || 0,
...
router.get('/', authenticate, requirePermission(PERMISSIONS.INVENTORY_READ), asyncHandler(async (req, res) => {
  const { search } = req.query as Record<string, string>;
  const where: Record<string, unknown> = { category: 'hospital', isActive: true };
  if (search) {
    where.OR = [
...
router.post('/:id/adjust', authenticate, requirePermission(PERMISSIONS.INVENTORY_WRITE), asyncHandler(async (req, res) => {
  ...
  const item = await prisma.inventoryItem.findFirst({ where: { id, category: 'hospital' } });
...
router.put('/:id', authenticate, requirePermission(PERMISSIONS.INVENTORY_WRITE), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { name, nameAr, sku, price, costPrice, minStock, expiryDate } = req.body;
  const existing = await prisma.inventoryItem.findFirst({ where: { id, category: 'hospital' } });
...
router.delete('/:id', authenticate, requirePermission(PERMISSIONS.INVENTORY_WRITE), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const existing = await prisma.inventoryItem.findFirst({ where: { id, category: 'hospital' } });
  if (!existing) throw new NotFoundError('Item not found');
