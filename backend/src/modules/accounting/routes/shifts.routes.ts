...
router.post('/open', authenticate, requirePermission(PERMISSIONS.ACCOUNTING_WRITE), auditMiddleware('OPEN_SHIFT', 'Shift'), asyncHandler(async (req, res) => {
...
router.post('/close', authenticate, requirePermission(PERMISSIONS.ACCOUNTING_WRITE), auditMiddleware('CLOSE_SHIFT', 'Shift'), asyncHandler(async (req, res) => {
  const shift = await prisma.shift.findFirst({ where: { userId: req.user!.id, closedAt: null }, include: { transactions: true } });
  if (!shift) throw new ValidationError('No open shift to close.');
  const { expectedTotal, actualTotal, denominations, notes } = req.body;
  const computedTotal = shift.transactions.reduce((acc, t) => acc + Number(t.amount), 0);
  const computedDenominations = denominations != null
    ? Object.entries(denominations).reduce((acc, [denom, qty]) => acc + Number(denom) * Number(qty), 0)
    : computedTotal;
  const closed = await prisma.shift.update({
    ...
    data: {
      ...
      actualTotal: actualTotal !== undefined ? actualTotal : computedDenominations,
